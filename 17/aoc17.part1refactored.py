...
def iterate2():
   ...
   theCount = 0
   for item in xdgrid:
      nbrcount = getNeighborCount([item[0],item[1],item[2]])
      prevState = xdgrid[get_key([item[0],item[1],item[2]],lookupIndex)][3]
      newState = 0
      if prevState ==1:
         ...
         if nbrcount == 3:
            newState=1
      theCount += newState
      newList.append([item[0],item[1],item[2],newState])
   xdgrid = newList
   print("ending iteration sum of " + str(theCount))
   return 1
